...
const ChangePasswordDialog = ({ isOpen, onClose }: ChangePasswordDialogProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      showToast.success(t('changePassword.passwordChanged'), t('changePassword.passwordChangedDesc'))
      ...
      <DialogContent className={`sm:max-w-md ${language === 'bn' ? 'font-bengali' : ''}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            >
              {isSubmitting ? t('changePassword.changing') : t('changePassword.changePassword')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
